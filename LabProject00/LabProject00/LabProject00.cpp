                     ...
                     _In_ int       nCmdShow)
{
    ...
    // 기본 메시지 루프입니다:
    while (GetMessage(&msg, nullptr, 0, 0)) // WM_QUIT(응용 프로그램 종료 시 발생) 메시지를 처리하기 전까지 무한 반복.
    {
...
ATOM MyRegisterClass(HINSTANCE hInstance)
{
    ...
    wcex.lpfnWndProc    = WndProc;  // 윈도우 프로시져를 설정함.
    wcex.cbClsExtra     = 0;
    wcex.cbWndExtra     = 0;
...
//
//  용도: 주 창의 메시지를 처리합니다.
//  메시지 루프에서 DispatchMessage() 윈도우 API 함수를 호출하면 WndProc() 함수가 실행됨.
//  윈도우 핸들, 메시지 ID, 2개의 파라메터로 매개변수 구성.
//
...
LRESULT CALLBACK WndProc(HWND hWnd, UINT message, WPARAM wParam, LPARAM lParam)
{
    ...
    case WM_PAINT:
        ...
        break;
    case WM_DESTROY:
        PostQuitMessage(0);     // WM_QUIT 메시지 생성.
        break;
    default:
        return DefWindowProc(hWnd, message, wParam, lParam);
    }
